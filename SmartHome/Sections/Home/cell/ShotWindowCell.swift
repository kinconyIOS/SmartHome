...
class ShotWindowCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        self.butting.layer.borderColor = UIColor.groupTableViewBackgroundColor().CGColor
        // Initialization code
    }
    ...
    @IBAction func ting(sender: AnyObject) {
        self.commad =  50
        ...
        if isMoni
        {
            ...
            self.butting.layer.borderColor = UIColor.greenColor().CGColor
            return
        }
        ...
        if isMoni
            {
                ...
                self.guan.layer.borderColor = UIColor.whiteColor().CGColor
                self.butting.layer.borderColor = UIColor.whiteColor().CGColor
                return
            }
            let dic = ["deviceAddress":address!,"command":self.commad]
            BaseHttpService.sendRequestAccess(commad_do, parameters: dic) { (back) -> () in
    ...
    @IBAction func shut(sender: AnyObject) {
        self.commad =  0
        let address = self.equip?.equipID
        if isMoni
        {
